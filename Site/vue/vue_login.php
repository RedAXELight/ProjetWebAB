...
<div>
    ...
    if (isset($resultats)) {
        ...
        if ($ligne['UserRole_idUserRole'] == '3') {
            echo "<h5> Bonjour " . $ligne['usrName'] . " " . $ligne['usrSurname'] . ". Vous êtes bien connecté en tant que client </h5>";
            // Création de la session
            $_SESSION['login'] = $ligne['usrName'] . " " . $ligne['usrSurname'];
            $_SESSION['typeUser'] = "Client";
        } else {
            if ($ligne['UserRole_idUserRole'] == '2') {
                echo "<h5> Bonjour " . $ligne['usrName'] . " " . $ligne['usrSurname'] . ". Vous êtes bien connecté en tant que vendeur </h5>";
                // Création de la session
                $_SESSION['login'] = $ligne['usrName'] . " " . $ligne['usrSurname'];
                $_SESSION['typeUser'] = "Vendeur";
            } else {
                if ($ligne['UserRole_idUserRole'] == '1') {
                    echo "<h5> Bonjour " . $ligne['usrName'] . " " . $ligne['usrSurname'] . ". Vous êtes bien connecté en tant qu'administrateur </h5>";
                    ...
                    <div class="form-group wow bounceInDown" data-wow-delay="0.4s">
                        <div class="form-group">
                            <form class='form' role="form" method='POST' data-toggle="validator"
                                  action="index.php?action=vue_login">
                                <table class="table table-hover">
                                    <tr>
                                        <td><label>Login : </label></td>
                                        <td>
                                            <input type="text" class="form-control" placeholder="Entrez votre login"
                                                   name="fLogin"
                                                   value="<?= @$_POST['fLogin'] ?>"/>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="password" class="form-control"
                                                   placeholder="Entrez votre mot de passe" name="fPass"
                                                   value="<?= @$_POST['fPass'] ?>"/>
                                        </td>
                                    </tr>
                                    <tr>
